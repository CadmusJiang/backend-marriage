...
# Marriage System 开发环境安装脚本
# 跳过数据库导入，只编译和启动应用

echo "=== Marriage System 开发环境安装脚本 ==="
...
if mysql -h $MYSQL_HOST -P $MYSQL_PORT -u $MYSQL_USER -p$MYSQL_PASS -e "SELECT 1;" > /dev/null 2>&1; then
    ...
    echo "⚠️  MySQL连接失败，但继续安装（开发环境）"
    echo "请确保数据库已正确配置，应用启动后会自动连接"
fi
