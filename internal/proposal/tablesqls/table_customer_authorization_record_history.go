...
func CreateCustomerAuthorizationRecordHistoryTableSql() (sql string) {
	...
	sql += "`history_id` varchar(64) NOT NULL COMMENT '历史记录ID',"
	sql += "`customer_authorization_record_id` varchar(64) NOT NULL COMMENT '客户授权记录ID',"
	...
	sql += "`is_deleted` enum('true','false') NOT NULL DEFAULT 'false' COMMENT '是否删除 true:是 false:否',"
