...
func CreateAccountHistoryTableSql() (sql string) {
	...
	sql += "`content` text NOT NULL COMMENT '操作内容 (JSON格式)',"
...
func CreateAccountHistoryTableDataSql() (sql string) {
	// 预置账户历史记录数据
	sql = "INSERT INTO `account_history` (`account_id`, `operate_type`, `operated_at`, `content`, `operator_username`, `operator_name`, `operator_role_type`, `created_user`, `updated_user`) VALUES"
	sql += "(2, 'modified', '2024-01-12 10:00:00', '{\"name\": {\"old\": \"张三\", \"new\": \"张明\"}, \"roleType\": {\"old\": \"员工\", \"new\": \"小队管理员\"}}', 'chenjing', '陈静', 'team_manager', '陈静', '陈静'),"
	sql += "(2, 'created', '2024-01-10 10:00:00', '{\"username\": {\"old\": \"\", \"new\": \"employee001\"}, \"name\": {\"old\": \"\", \"new\": \"张三\"}, \"roleType\": {\"old\": \"\", \"new\": \"员工\"}, \"phone\": {\"old\": \"\", \"new\": \"13800138000\"}, \"belongGroup\": {\"old\": \"\", \"new\": \"南京-天元大厦组\"}, \"belongTeam\": {\"old\": \"\", \"new\": \"营销团队A\"}}', 'admin', '系统管理员', 'company_manager', '系统管理员', '系统管理员');"
